...
const GlassCard: React.FC<CardProps> = ({ title, content, image, onClose }) => {
  ...
  return (
	...
	>
		...
		<button className="absolute top-8 right-8 text-white hover:text-[#28282b]/50 z-10 cursor-pointer" onClick={onClose}>
         	<FaGear size={24} />
        </button>

		<div 
			className="bg-white/10 h-fit rounded-3xl md:w-[50%] lg:w-[40%] border border-[#28282b]/10"
			onClick={(e) => e.stopPropagation()}
		>
	    	<img src={image} alt={title} className="w-full h-64 object-cover rounded-t-3xl" />

			<div className="p-6 flex flex-col">
    	       <h2 className="text-3xl font-bold text-[#28282b] my-1 source_code_pro">{title}</h2>
